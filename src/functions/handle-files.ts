...
export function readEndpointFile(
  ...
  return new Promise((resolve) => {
    ...
    fs.readFile(filePath, 'utf8', async function (err, data) {
      ...
      /** Experimental parser. Used to get variables In the future, will be used to get other patterns */
      ...
      /** DataToGetPatterns: this variable will be used to get patterns before of method, such as: app, route, etc. */
      ...
      /** Bugfix when HTTP methods are at the end of 'dataToGetPatterns' Issue: #49 */
      ...
      /** CASE: import UserRouters from "./user"; ... router.use("/", new UserRouters().routes); */
      ...
      /**
       * Eliminating unwanted patterns within endpoints Avoinding cases, such as: route.get('/path', ...
       * ...query().delete().where(...); whithin of the endpoint's functions make problems because of the '.delete()'
       */
      let aDataAux = aData;
      let finished = false;
      ...
      if (aData.includes(statics.SWAGGER_TAG + '.patterns')) {
        /** Manual pattern recognition NOTE: Deprecated */
      ...
      /** CASE: router.use(middleware).get(...).post(...).put(...)... REFACTOR: pass to function */
      ...
      if (importedFiles.length > 0) {
        ...
        if (idx > -1) {
          ...
          let varRouteFound = aData
            .replaceAll('\n', '')
            .split(
              new RegExp(
                `(const|let|var)(\\s+\\w+\\s*\\t*\\=\\s*\\t*${varName}\\s*\\t*\\.\\s*\\t*Router\\s*\\t*\\(\\s*\\t*\\))`,
              ),
            );
          varRouteFound.map((d) => {
          ...
          let varExpressRouteFound = aData
            .replaceAll('\n', '')
            .split(new RegExp(`(\\s+\\t*${varName}\\s*\\t*\\.\\s*\\t*Router\\s*\\t*\\(\\s*\\t*\\))`));
          if (
            varExpressRouteFound[1] &&
            varExpressRouteFound[2] &&
            varExpressRouteFound[2].split(/^____CHAINED____/).length > 1
          ) {
            ...
            if (regex == '') {
              ...
              aData = aData.replaceAll(
                new RegExp(`\\s+\\t*${varName}\\s*\\t*\\.\\s*\\t*Router\\s*\\t*\\(\\s*\\t*\\)____CHAINED____`),
                ` ${routerVar}`,
              );
      ...
      if (regex != '' || aForcedsEndpoints.length > 0) {
        ...
        for (let idxElem = 0; idxElem < aData.length; idxElem++) {
          ...
          /**
           * CASE (continuing): router.use(middleware).get(...).post(...).put(...)... Adding middleware to be processed
           * together with the other endpoint functions
           */
          ...
          if (swaggerTags.getIgnoreTag(elem)) {
            ...
            swaggerTags.getIgnoreTag(globalSwaggerProperties) &&
            elem &&
            elem.split(new RegExp(`${statics.SWAGGER_TAG}.ignore\\s*\\=`)).length == 1
          ) {
            continue;
          }
          ...
          if ((elemParam && elemParam.split(',').length > 1 && !forced) || predefMethod === 'use') {
            ...
            if (auxElem.split(',').length > 1 || predefMethod === 'use') {
              let found = expressVarName.findIndex(
                (v) => auxElem.split(new RegExp(`\\,\\s*\\n*\\t*${v}\\s*\\n*\\t*\\)`)).length > 1,
              );
              ...
              /**
               * Handling foo.method('/path', ..., ...)' Getting function not referenced ( such as: (req, res) => { ...
               * } )
               */
              ...
              for (let idxFunc = 0; idxFunc < 15; ++idxFunc) {
                ...
                if (predefMethod == 'use' && funcNotReferenced) {
                  ...
                  /** CASE: app.method("/foo", (req, res) => { foo(req, res); }); */
            ...
            for (let index = 0; index < functions.length; index++) {
              ...
              if (rexRequire.test(func)) {
                ...
                /** CASE: awilix-express const fooFoo = require('./pathToFoo') ... router.method('/path', fooFoo('foo') ) */
                if (func.includes('(') && func.includes(')')) {
                ...
                if (idx == -1) {
                  ...
                  for (let idxImp = 0; idxImp < importedFiles.length; ++idxImp) {
                    ...
                    let found =
                      imp && imp.exports
                        ? imp.exports.find((e) => e.varAlias && varFileName && e.varAlias == varFileName)
                        : null;

                    if (!found) {
                      found =
                        imp && imp.exports
                          ? imp.exports.find((e) => e.varName && varFileName && e.varName == varFileName)
                          : null;
                    } else {
                    ...
                    if (found) {
                      ...
                      if (
                        exportPath &&
                        imp.isDirectory &&
                        (await utils.getExtension(exportPath)) == '' &&
                        (await utils.getExtension(exportPath + '/index')) != ''
                      ) {
              ...
              if (idx > -1 || exportPath) {
                  ...
                  if (dataIndexFile) {
                    ...
                    if (idx == -1) {
                      importsIndexFile.forEach((imp) => {
                        ...
                        let found =
                          imp && imp.exports
                            ? imp.exports.find((e) => e.varName && functionName && e.varName == functionName)
                            : null;
                        if (found) {
                          if (!functionName) {
                            functionName = found.varName;
                ...
                if (!refFunction && refFuncInParamStr) {
                  ...
                  if (fileContent && fileContent.includes('awilix-express'))
                    refFunction = await functionRecognizerInFile(pathFile + extension, refFuncInParamStr);
                }

                /** CASE: Reference to files in the index.ts Ref.: issue #32 */
                if (!refFunction) {
                  if (!functionName) {
                    let dataIndexFile = await utils.getFileContent(pathFile + extension);
                    if (dataIndexFile) {
                      ...
                      /**
                       * 'hidding' imports and catching only exports and change exports to imports to catched by the
                       * getImportedFiles()
                       */
                      ...
                      /**
                       * TODO: searching in the varFileName let idx = exportsIndexFile.findIndex(e => e.varFileName &&
                      ...
                      if (idx == -1) {
                        exportsIndexFile.forEach((imp) => {
                          ...
                          let found =
                            imp && imp.exports ? imp.exports.find((e) => e.varAlias && e.varAlias == 'default') : null;
                          if (found) {
                        ...
                        if (exported.exports) {
                          for (let localIdxExp = 0; localIdxExp < exported.exports.length; ++localIdxExp) {
                            ...
                            if (localExported && localExported.varName && localExported.varName == functionName) {
                              ...
                              if (
                                exported.fileName &&
                                exported.fileName.split(new RegExp('\\.js$')).length > 1 &&
                                !fs.existsSync(exported.fileName)
                              ) {
                ...
                let refFunction = await handleData.functionRecognizerInData(
                  aDataRaw,
                  varFileName === 'this' ? functionName : varFileName,
                );
          ...
          if (endpointFunctions && endpointFunctions.length == 0) {
            ...
            for (let _idxEF = 0; _idxEF < endpointFunctions.length; ++_idxEF) {
              ...
              if (swaggerTags.getIgnoreTag(endpoint)) {
                ...
                swaggerTags.getIgnoreTag(globalSwaggerProperties) &&
                endpoint &&
                endpoint.split(new RegExp(`${statics.SWAGGER_TAG}.ignore\\s*\\=`)).length == 1
              ) {
                break;
              }
              ...
              if (endpoint && endpoint.includes(statics.SWAGGER_TAG + '.summary')) {
                ...
                globalSwaggerProperties &&
                globalSwaggerProperties.includes(statics.SWAGGER_TAG + '.summary')
              ) {
              ...
              if (
                ...
                globalSwaggerProperties &&
                globalSwaggerProperties.includes(statics.SWAGGER_TAG + '.parameters') &&
                globalSwaggerProperties.includes('[') &&
                endpoint.includes(']')
              ) {
              ...
              if (endpoint && endpoint.includes(statics.SWAGGER_TAG + '.requestBody')) {
                ...
                globalSwaggerProperties &&
                globalSwaggerProperties.includes(statics.SWAGGER_TAG + '.requestBody')
              ) {
              ...
              if (!swaggerTags.getOpenAPI()) {
                if (endpoint && endpoint.includes(statics.SWAGGER_TAG + '.produces')) {
                  ...
                  globalSwaggerProperties &&
                  globalSwaggerProperties.includes(statics.SWAGGER_TAG + '.produces')
                ) {
                ...
                if (endpoint && endpoint.includes(statics.SWAGGER_TAG + '.consumes')) {
                  ...
                  globalSwaggerProperties &&
                  globalSwaggerProperties.includes(statics.SWAGGER_TAG + '.consumes')
                ) {
              ...
              if (endpoint && endpoint.includes(statics.SWAGGER_TAG + '.description')) {
                ...
                globalSwaggerProperties &&
                globalSwaggerProperties.includes(statics.SWAGGER_TAG + '.description')
              ) {
              ...
              if (endpoint && endpoint.includes(statics.SWAGGER_TAG + '.security')) {
                ...
                globalSwaggerProperties &&
                globalSwaggerProperties.includes(statics.SWAGGER_TAG + '.security')
              ) {
              ...
              if (endpoint && endpoint.includes(statics.SWAGGER_TAG + '.deprecated')) {
                ...
                globalSwaggerProperties &&
                globalSwaggerProperties.includes(statics.SWAGGER_TAG + '.deprecated')
              ) {
              ...
              if (autoMode && endpoint && ((req && req.length > 0) || (res && res.length > 0))) {
                ...
                if (res) {
                  ...
                  objEndpoint = handleData.getHeader(
                    await handleData.removeComments(rawEndpoint),
                    path,
                    method,
                    res,
                    objEndpoint,
              ...
              if (globalObjResponses) {
                Object.keys(globalObjResponses).forEach((r) => {
                  if (objResponses[r] && globalObjResponses[r].ifStatusPresent === true) {
                    ...
                    if (
                      globalObjResponses[r].description &&
                      (!objResponsesTag ||
                        (globalObjResponses[r].description &&
                          objResponsesTag[r] &&
                          objResponses[r] &&
                          objResponsesTag[r].description != objResponses[r].description))
                    ) {
              ...
              Object.values(objParameters).forEach((objParam) => {
                if (objEndpoint[path][method].parameters) {
                  ...
                  let idxFound = objEndpoint[path][method].parameters.findIndex(
                    (e) => e && e.name === objParam.name && e.in === objParam.in,
                  );
                  if (objParam.name) {
              ...
              if (
                ...
                objEndpoint[path][method].parameters = currentParameters.filter((e, pIdx, a) => {
                  let idxFound = a.findIndex(
                    (i) =>
                      i &&
                      ((i.in && e.in && i.name && e.name && i.in === e.in && i.name === e.name) ||
                        (i[ref] && e[ref] && i[ref] == e[ref])),
                  );
              ...
              if (
                ...
                if (objEndpoint[path][method].parameters && objEndpoint[path][method].parameters.length > 0) {
                  objEndpoint[path][method].parameters = objEndpoint[path][method].parameters.filter(
                    (p) => p.in && p.in.toLowerCase() !== 'body',
                  );
                }
              } else if (objInBody && _idxEF == endpointFunctions.length - 1) {
                /**
                 * If #swagger.parameter or #swagger.requestBody is present the automatic body recognition will be
                 * ignored.
                 */
      ...
      if (aRoutes && aRoutes.length >= 1) {
        for (let file = 0; file < aRoutes.length; file++) {
          ...
          if (
            (!routeSwaggerProperties || routeSwaggerProperties == '') &&
            globalSwaggerProperties &&
            globalSwaggerProperties.replaceAll(' ', '') != ''
          ) {
          ...
          if (
            ...
            rawPath.includes('.') &&
            importedFiles.find(
              (e) => e.varFileName && rawPath.split('.')[0].trim() && e.varFileName == rawPath.split('.')[0].trim(),
            )
          ) {
            ...
            if (found) {
              ...
              if (
                found.fileName &&
                found.fileName.split(new RegExp('\\.js$')).length > 1 &&
                !fs.existsSync(found.fileName)
              ) {
          ...
          if (data.split(',').length == 1) {
            ...
            if (data.trim()[0] === ',' && (await utils.stack0SymbolRecognizer(data, '[', ']'))) {
              /** Multiple express Routers under the same name space Issue: 61 */
            ...
            for (let idxFileName = 0; idxFileName < listOfFileName.length; ++idxFileName) {
              /** CASE: import fooFoo from "./pathToFoo"; ... router.use("/", new fooFoo().foo); */
          ...
          for (let idxFileName = 0; idxFileName < listOfFileName.length; ++idxFileName) {
            ...
            if (
              obj.varFileName &&
              obj.varFileName.split(new RegExp('\\:|\\;|\\=|\\>|\\<|\\{|\\}|\\(|\\)|\\[|\\]|\\,')).length > 1
            ) {
            ...
            if (idx == -1 && !exportPath) {
              ...
              importedFiles.forEach((imp, importIdx) => {
                ...
                let found =
                  imp && imp.exports
                    ? imp.exports.find((e) => e.varName && obj.varFileName && e.varName == obj.varFileName)
                    : null;
                if (found) {
              ...
              if (found) {
                ...
                if (content) {
                  ...
                  let varRouteFound = content
                    .split(new RegExp('(const|let|var)(\\s+\\w+\\s*\\=\\s*\\n*\\t*Router\\s*\\n*\\t*\\(\\))'))
            ...
            if (idx > -1 || exportPath) {
              ...
              if (idx > -1 && importedFiles[idx] && importedFiles[idx].isDirectory && !exportPath) {
                ...
                if (
                  obj.fileName &&
                  obj.fileName.split(new RegExp('\\.js$')).length > 1 &&
                  !fs.existsSync(obj.fileName)
                ) {
 ...
 */
export async function getImportedFiles(data, localRelativePath) {
  let importedFiles = [];
  try {
    ...
    if (importeds && importeds.length > 1) {
      ...
      if (tsconfig) {
        ...
        tsBaseUrl =
          ...
          typeof tsconfig.compilerOptions.baseUrl === 'string'
            ...
            : null;
        let rootDir =
          tsconfig.compilerOptions && tsconfig.compilerOptions.rootDir ? tsconfig.compilerOptions.rootDir : '';
        if (tsRelativePath) {
      ...
      for (let index = 0; index < importeds.length; ++index) {
        ...
        fileName && fileName.split(new RegExp(' from |\\}\\s*from\\s*\\"?\\\'?\\`?', 'i')).length > 1
        ...
        /** CASE: Cannot resolve import function from absolute path Issue: #50 */
              ...
              if (dataFile) {
                ...
                for (let idx = 0; idx < obj.exports.length; ++idx) {
                  ...
                  let idxFound = imports.findIndex(
                    (e) => e.varFileName && varName && e.varFileName.toLowerCase() == varName.toLowerCase(),
                  );
                  let exportPath = null;
                  if (idxFound == -1) {
                    imports.forEach((imp) => {
                      ...
                      let found =
                        imp && imp.exports
                          ? imp.exports.find(
                              (e) => e.varName && varName && e.varName.toLowerCase() == varName.toLowerCase(),
                            )
                          : null;
                      if (found) {
                        if (imp.isDirectory) {
              ...
              if (indexExtension != '') {
                ...
                if (isRequireDirLib) {
                  ...
                  for (let pathIdx = 0; pathIdx < obj.exports.length; ++pathIdx) {
                    ...
                    if (
                      dataFile.split(new RegExp(`${oExp.varName}\\s*\\n*\\t*\\=\\s*\\n*\\t*require\\s*\\n*\\t*\\(`))
                        .length > 1
                    ) {
                      let addPath = dataFile.split(
                        new RegExp(`${oExp.varName}\\s*\\n*\\t*\\=\\s*\\n*\\t*require\\s*\\n*\\t*\\(\\s*\\n*\\t*`),
                      );
 ...
 *
 * @param {any} filePath File's path.
 * @param {any} functionName
 * @param {any} isRecursive To avoid infinite loop in case of recursion.
 */
export function functionRecognizerInFile(filePath, functionName, isRecursive = true) {
  return new Promise((resolve) => {
    fs.readFile(filePath, 'utf8', async function (err, data) {
      if (err) {
        return resolve(null);
      ...
      try {
        ...
        if (functionName) {
          ...
          /**
           * CASE: Referenced function, such as: module.exports = { foo: require('./fooFile').foo } in index file Issue:
           * #29
           */
          if (
            !funcStr &&
...
export async function resolvePathFile(path, relativePath) {
  let solvedPath = path;
