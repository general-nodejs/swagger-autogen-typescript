...
import { Schema } from 'types/v2Types';
...
// TODO: split this
export function formatDefinitions(def: { [key: string]: Schema }, resp = {}, containXML: boolean) {
  try {
    // Forcing conversion to OpenAPI 3.x
    ...
    if (def && def.$ref) {
      if (def.$ref.split('#/').length === 1) {
        console.error('[swagger-autogen-typescript]: Syntax error: ', def.$ref);
        return resp;
      }
    ...
    if (typeof def === 'string') {
      ...
      Object.entries(def).forEach((elem) => {
        if (typeof elem[1] === 'object') {
          ...
          if (resp.type == 'array') {
            resp.items = {
              ...formatDefinitions(elem[1], resp, containXML),
            };
          } else {
            resp.properties[elem[0]] = formatDefinitions(elem[1], resp, containXML);
          }
        } else {
          if (resp.type == 'array') {
            if (arrayOf == 'object') {
 ...
 *
 * @param elem - The line of code that contains the path
 * @param autoMode - True
 * @returns The path of the route.
 */
export function getPath(elem, autoMode) {
  if (!elem) {
    return null;
  }
 ...
 * @returns
 */
export function getMethodTag(data, reference) {
  try {
    ...
    console.error(
      `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.method' out of structure in\n... '${reference.filePath}'`,
    );
    return false;
  }
}
 ...
 * @param {any} aData
 */
export function getForcedEndpoints(aData, reference) {
  try {
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '#swagger.start' ... '#swagger.end' out of structure in\n... '${reference.filePath}'`,
      );
    }
    return [];
  }
 ...
 * @param {any} elem
 */
export function getIgnoreTag(elem) {
  try {
 ...
 * @param {any} data
 */
export function getAutoTag(data) {
  try {
 ...
 * @param {any} data
 */
export function getDeprecatedTag(data, reference) {
  try {
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.deprecated' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
      );
    }
    return false;
  }
 ...
 */
export async function getParametersTag(data, objParameters, reference) {
  ...
  try {
    ...
    for (let idx = 0; idx < swaggerParameters.length; ++idx) {
      ...
      try {
        ...
        console.error('[swagger-autogen-typescript]: Syntax error: ' + parameter);
        console.error(
          `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.parameters' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
        );
      ...
      if (objParameters[name] && objParameters[name]['@schema']) {
        ...
        if (objParameters[name].schema['@enum']) {
          ...
          !(Object.keys(objParameters[name].schema).length == 1 && objParameters[name].schema.type) &&
          (!objParameters[name].schema.properties ||
            (!objParameters[name].schema.properties['__AUTO_GENERATE__'] &&
              objParameters[name].schema.properties &&
              Object.keys(objParameters[name].schema.properties).length > 0))
        ) {
 ...
 * @param {string} data File content
 */
export async function getRequestBodyTag(data, reference) {
  try {
    let requestBody = {};
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.requestBody' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
      );
    }
    return {};
  }
 ...
 * @param {any} data
 */
export async function getProducesTag(data, reference) {
  try {
    let produces = [];
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.produces' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
      );
    }
    return [];
  }
 ...
 * @param {any} data
 */
export async function getConsumesTag(data, reference) {
  try {
    let consumes = [];
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.consumes' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
      );
    }
    return [];
  }
 ...
 *
 * @param {any} data
 * @param {any} objResponses
 */
export async function getResponsesTag(data, objResponses, reference) {
    ...
    for (let idx = 0; idx < swaggerResponses.length; ++idx) {
      ...
      if (
        swaggerResponses[idx].split(
          new RegExp(`\\[\\s*\\t*\\s*\\t*${statusCode}\\s*\\t*\\s*\\t*\\]\\s*\\t*\\s*\\t*\\=\\s*\\t*\\s*\\t*\\{`),
        ).length > 1
      ) {
        // has object
        ...
        try {
          ...
          console.error('[swagger-autogen-typescript]: Syntax error: ' + objResp);
          console.error(
            `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.responses' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
          );
 ...
 * @param {any} data
 */
export function getDescription(data, reference) {
  try {
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.description' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
      );
    }
    return '';
  }
 ...
 * @param {any} data
 */
export function getSummary(data, reference) {
  try {
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.summary' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
      );
    }
    return '';
  }
 ...
 * @param {any} data
 */
export function getOperationId(data, reference) {
  try {
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.operationId' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
      );
    }
    return '';
  }
 ...
 * @param {any} data
 */
export function getTags(data, reference) {
  try {
    let tags = [];
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.tags' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
      );
    }
    return [];
  }
 ...
 * @param {any} data
 */
export async function getSecurityTag(data, reference) {
  try {
    let security = [];
    ...
    if (!getDisableLogs()) {
      console.error(
        `[swagger-autogen-typescript]: '${statics.SWAGGER_TAG}.security' out of structure in\n'${reference.filePath}' ... ${reference.predefPattern}.${reference.method}('${reference.path}', ...)`,
      );
    }
    return [];
  }
