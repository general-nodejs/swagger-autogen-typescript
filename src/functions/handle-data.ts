...
/**
 * Convert statements such as: "require('./path')(foo)" in "foo.use(require('./path'))" Useful, because the statement
 * "foo.use(require('./path'))" is already handled successfully.
 *
 ...
 * @param {string} data File content.
 */
export function removeAwaitsAndAsyncs(data) {
  const origData = data;
  try {
    ...
    for (let idx = 0; idx < data.length; idx += 1) {
      ...
      if (
        data.slice(idx, idx + 6) === 'await ' &&
        idx > 0 &&
        data[idx - 1].split(new RegExp('([a-z]|[A-Z]|[0-9]|_|$)')).length === 1
      ) {
        // Remove await keyword
        data = data.slice(0, idx) + data.slice(idx + 6);
        continue;
      } else if (
        data.slice(idx, idx + 5) === 'async' &&
        idx > 0 &&
        data[idx - 1].split(new RegExp('([a-z]|[A-Z]|[0-9]|_|$)')).length === 1 &&
        data[idx + 5].split(new RegExp('([a-z]|[A-Z]|[0-9]|_|$)')).length === 1
      ) {
 ...
 */
export function removeComments(data, keepSwaggerTags = false) {
  return new Promise((resolve) => {
    if (!data || data.length == 0) {
      return resolve(data);
    }
 ...
 * @param {string} data File content.
 */
export function getSwaggerComments(data) {
  return new Promise((resolve) => {
    if (data.length == 0) {
      return resolve(data);
    }
    ...
    try {
      for (let idx = 0; idx < data.length; ++idx) {
        ...
        if (stackComment1 == 0 && stackComment2 == 0) {
          ...
          // Type  "
          if (c == '"' && (data[idx - 1] != '\\' || (data[idx - 1] == '\\' && data[idx - 2] == '\\')) && isStr2 == 1)
            isStr2 = 2;
          if (
            c == '"' &&
            (data[idx - 1] != '\\' || (data[idx - 1] == '\\' && data[idx - 2] == '\\')) &&
          ...
          // Type  `
          if (c == '`' && (data[idx - 1] != '\\' || (data[idx - 1] == '\\' && data[idx - 2] == '\\')) && isStr3 == 1)
            isStr3 = 2;
          if (
            c == '`' &&
            (data[idx - 1] != '\\' || (data[idx - 1] == '\\' && data[idx - 2] == '\\')) &&
 ...
 * @param {string} data File content.
 */
export function removeStrings(data) {
  return new Promise((resolve) => {
    if (!data || data.length == 0) {
      return resolve(data);
    }
 ...
 */
export function removeInsideParentheses(data, keepParentheses = false, level = 0) {
  return new Promise((resolve) => {
    if (data.length == 0) {
      return resolve(data);
    }
      ...
      for (let idx = 0; idx < data.length; ++idx) {
        ...
        if (c == ')' && data[idx - 1] != '\\') {
          ...
          if (stack == level) {
            ...
            if (validChar == '(') {
              /** Recognize middlewares in parentheses Issue: #67 */
              strToReturn += buffer;
            }
            buffer = '';
          }
 ...
 */
export function addReferenceToMethods(data, patterns) {
  return new Promise((resolve) => {
    if (!data) {
      return resolve(data);
    }
 ...
 * @param {any} elem
 * @param {any} request
 * @param {any} objParameters
 */
export function getQueryIndirectly(elem, request, objParameters) {
  ...
  try {
    for (let idx = 0; idx < request.length; ++idx) {
      ...
      if (
        req &&
        req.split(new RegExp('\\;|\\{|\\(|\\[|\\"|\\\'|\\`|\\}|\\)|\\]|\\:|\\,|\\*|\\!|\\|')).length == 1 &&
        elem &&
        elem.split(new RegExp(' .*?\\s*\\t*=\\s*\\t*' + req + '\\.\\s*\\t*query(\\s|\\n|;|\\t)', 'gmi').length > 1)
      ) {
        let queryVars = [];
        ...
        if (aQuerys.length > 0) {
          ...
          if (queryVars.length > 0) {
            queryVars.forEach((query) => {
              if (
                query &&
                query.split(new RegExp('\\;|\\{|\\(|\\[|\\"|\\\'|\\`|\\}|\\)|\\]|\\:|\\,|\\*|\\!|\\|')).length == 1
              ) {
 ...
 */
export async function getStatus(elem, response, objResponses) {
        ...
        elem
          ...
          .forEach(async (s) => {
            ...
            if (status && !utils.isNumeric(status) && status.split(new RegExp('\\?|\\|\\||\\:')).length > 1) {
              /** CASE: Handle status function (Express.js) with variables or multiple status code Issue: #62 */
 ...
 */
export function getHeader(elem, path, method, response, objEndpoint) {
 ...
 */
export function getHeaderQueryBody(elem, request, objParameters) {
  const origObjParameters = objParameters;
    ...
    for (let idx = 0; idx < request.length; ++idx) {
      ...
      try {
        ...
        for (let idxInput = 0; idxInput < inputTypes.length; ++idxInput) {
          ...
          let splitedElemTS = elem.split(
            new RegExp(`(\\s+\\w+\\s*\\:\\s*\\w+\\s*\\=\\s*${req}\\s*\\.\\s*${inputType}(?!\\.))`),
          );
        ...
        elem
          ...
          .forEach((p) => {
            ...
            if (
              ...
              (objParameters[name] && objParameters[name].name === name && objParameters[name].in !== 'header')
            ) {
      ...
      /** Headers Destructuring in body, such as: {a, b} = req.headers */
      if (
        globalOptions.autoHeaders &&
        req &&
        elem &&
      ...
      /** Headers E.g: let someHeader = req.headers['x-token'] */
      if (globalOptions.autoHeaders && req && elem && elem.split(' ' + req + '.headers[').length > 1) {
        ...
        elem
          ...
          .forEach((p) => {
            ...
            if (
              !!objParameters[name] === false ||
              (objParameters[name] && objParameters[name].name !== name) ||
              (objParameters[name] && objParameters[name].name === name && objParameters[name].in !== 'query')
            ) {
      ...
      /**
       * Pull Request (#30) CASE: Destructuring in body, such as: {a, b} = req.query Created by: WHL Modified by: Davi
       * Baltar
       */
      if (
        globalOptions.autoQuery &&
        req &&
        elem &&
        ...
        for (let idxQuery = 0; idxQuery < elems.length - 1; ++idxQuery) {
          ...
          /**
           * CASE: const { item1, item2: { subItem1, subItem2 } } = req.query; Solution: Eliminate sub-items for now
      ...
      /** Pull Request (#30) CASE: Recognize body parameters Created by: WHL Modified by: Davi Baltar */
      ...
      /**
       * Pull Request (#30) CASE: Destructuring in body, such as: {a, b} = req.body Created by: WHL Modified by: Davi
       * Baltar
       */
      if (
        globalOptions.autoBody &&
        req &&
        elem &&
        elem.split(new RegExp('\\}\\s*\\n*\\t*\\=\\s*\\n*\\t*' + req + '.body\\s+')).length > 1
      ) {
        ...
        for (let idxBody = 0; idxBody < elems.length - 1; ++idxBody) {
          ...
          /**
           * CASE: const { item1, item2: { subItem1, subItem2 } } = req.body; Solution: Eliminate sub-items for now
 ...
 */
export async function getCallbackParameters(data) {
  if (!data) {
    return {
      req: [],
      res: [],
 ...
 * @param {object} objParameters
 */
export async function getPathParameters(path, objParameters) {
  if (!path) {
    return objParameters;
  }
  const origObjParameters = objParameters;
...
/**
 * TODO: REFACTOR THIS FUNCTION Recognize function in a string data.
 *
 * @param {string} data Content.
 * @param {string} functionName
 */
export async function functionRecognizerInData(data, functionName) {
 ...
 * @param {string} data Content.
 */
export async function popFunction(data) {
  if (!data) {
    return null;
  }
