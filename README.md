...
[![NPM Version](http://img.shields.io/npm/v/swagger-autogen-typescript.svg?style=flat)](https://www.npmjs.com/package/swagger-autogen-typescript)
[![NPM Downloads](https://img.shields.io/npm/dm/swagger-autogen-typescript.svg?style=flat)](https://npmcharts.com/compare/swagger-autogen-typescript?minimal=true)
...
```bash
$ npm install --save-dev swagger-autogen-typescript
```

If you're using CommonJS:

```js
const swaggerAutogen = require('swagger-autogen-typescript')();
```

Or if you're using ES modules:

```js
import swaggerAutogen from 'swagger-autogen-typescript';
```
...
```bash
$ npm install --save-dev swagger-autogen-typescript@latest
```

## Usage

[Example using Router](https://github.com/davibaltar/example-swagger-autogen-typescript-with-router)

[Example without Router](https://github.com/davibaltar/example-swagger-autogen-typescript)
...
The code below must be inserted in a separate file (e.g _swagger.js_):
...
```js
    ...
    "scripts": {
        // ... ,
        "swagger-autogen-typescript": "node ./swagger.js"
    }
```
...
```bash
$ npm run swagger-autogen-typescript
```
...
See: [Complete example](https://github.com/davibaltar/example-swagger-autogen-typescript)
...
```js
const swaggerAutogen = require('swagger-autogen-typescript')(options);
```
...
**OpenAPI:** To enable OpenAPI v3, assign a version, such as "3.0.0" to the _openapi_ parameter.
...
```js
    app.post('/path', (req, res) => {
        ...
        const myItem1 = req.body.item1  // Will be ignored by swagger-autogen-typescript

        const { item2, item3 } = req.body  // Will be ignored by swagger-autogen-typescript
        ...
    })
```
...
It is possible to create or complement automatically detected responses. Use the `#swagger.reponses[statusCode]` tag to create a new answer or to complete an existing answer (automatically detected). If **OpenAPI 3.x** option is enabled ([see how to enable OpenAPI 3.x option](#options)), the _content_ will be "application/json" by default. To see more options about OpenAPI 3.x response, see it here [OpenAPI 3.x response](#responses-1).
...
To ignore the swagger-autogen-typescript render in the `definitions` (Swagger 2.0) or `components.schemas` (OpenAPI 3) and put the specification Swagger directly, you can use `@definitions` or `@schemas`, such as:
        ...
        someParameter: {
            ...
            properties: {
                property1: {
                    type: 'integer',
                    format: 'int32',
                    description: 'With no swagger-autogen-typescript render...'
                }
            }
        }
    }
...
Use the `'@schema'` instead of `schema` if you don't want swagger-autogen-typescript to do schema processing. In this case you must build the schema according to Swagger's specs. The result in the _.json_ will be the same in `'@schema'`.
...
_NOTE:_ The implementation of OpenAPI 3.x features are in progress. Some features are not yet available in the lib.
...
```js
const swaggerAutogen = require('swagger-autogen-typescript')({ openapi: '3.0.0' });
```
...
Use the `'@content'` instead of `content` if you don't want swagger-autogen-typescript to do the content processing. In this case you must build the content according to Swagger's specs. The result in the _.json_ will be the same in `'@content'`, for example:
...
```js
const swaggerAutogen = require('swagger-autogen-typescript')({ language: 'pt-BR' });
// In this case, for example, the description of status code 404 will be:
// 'Não Encontrado'
```
...
```js
const swaggerAutogen = require('swagger-autogen-typescript')({ language: 'zh-CN' });
// In this case, for example, the description of status code 404 will be:
// '未找到'
```
...
```js
const swaggerAutogen = require('swagger-autogen-typescript')({ language: 'ko' });
// In this case, for example, the description of status code 404 will be:
// '찾을 수 없음'
```
...
```js
const swaggerAutogen = require('swagger-autogen-typescript')({ language: 'fr' });
// In this case, for example, the description of status code 404 will be:
// 'Non Trouvé'
```
...
Help us improve this module. If you have any information that the module does not provide or provides incompletely or incorrectly, please use our [Github](https://github.com/davibaltar/swagger-autogen-typescript) repository.
